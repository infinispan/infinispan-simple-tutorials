...
[source]
----
$ export TEST=SIMPLEAUTH1,SIMPLEAUTH2 && {exec_run}
----
...
[source]
----
$ export TEST=CLIENT1,CLIENT2 && {exec_run}
----
+
. Check server logs to verify that the PUT and GET requests succeed.
...
[source]
----
$ export TEST=CLIENT1AUTH,CLIENT2AUTH && {exec_run}
----
...
Consider that running the client needs `mvn {exec_run}` to not fail as there are multiple executions defined.
